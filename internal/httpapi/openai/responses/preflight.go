...
func (h *Handler) preflightParsedResponsesRequest(w http.ResponseWriter, r *http.Request, req map[string]any) bool {
	if files.HasInlineUploadPayload(req) {
		return true
	}
	stdReq, err := promptcompat.NormalizeOpenAIResponsesRequest(h.Store, req, requestTraceID(r))
	if err != nil {
