...
- **调用方并发与内容预检** (`caller_max_inflight`、`max_prompt_chars`、`max_ref_files_per_request`、`max_inline_files_per_request`、`prompt_risk_guard_enabled`、`prompt_block_rules`)：默认限制单个客户端凭据并发为 `2`，并在触达 DeepSeek 前拦截过长 prompt、过多引用文件、inline 文件和已知高风险 prompt 指纹；指纹命中会本地返回 `422 prompt_blocked`。
